...
const ListMedia = () => {
    ...
    <div className='flex flex-col gap-1.5'>
        {state.videoGroups.map(group => (
            <button key={group.id} className='btn btn-primary btn-xs m-0.5' onClick={() => onClickVideo(group)}>
                {group.name}
                {group.muted ? 
        ...
        {state.audioGroups.map(group => (
            <button key={group.id} className='btn btn-primary btn-xs m-0.5' onClick={() => onClickAudio(group)}>
                {group.name}
                {group.muted ? 
                <img src={Muted} className='ml-auto'/> :
                <img src={Sound} className='ml-auto'/> }
